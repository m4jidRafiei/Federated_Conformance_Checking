...
val attr = ["Case ID", "Activity","Org:resource","Start Timestamp","Complete Timestamp","Output","Input"];
val minute = 60.0;
...
fun event(i:int,tes:string,res:string,re:real,Output:string, Input:string)= write_record([Int.toString(i),tes,res, t2s(toReal(Mtime())),t2s(toReal(Mtime())+re),Output, Input]);
